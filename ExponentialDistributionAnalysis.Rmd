...
sim.sd = sd( sim.means );
```
The theoretical variance of the sample is given by 
...
that the sample mean variance and theoretical variance would similar, as they
...
### Distribution
As can be seen from the following histrogram, random values drawn from the
exponential distribution do not have a normal distribution.
...
print( sim.histogram );
```
The central limit theorm states that
